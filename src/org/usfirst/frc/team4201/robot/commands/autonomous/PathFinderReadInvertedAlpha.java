 ...
 *
 */
public class PathFinderReadInvertedAlpha extends Command {
	...
	public PathFinderReadInvertedAlpha(String filename, boolean first, double maxVel) {
    ...
    public PathFinderReadInvertedAlpha(String filename, boolean first) {
        // Use requires() here to declare subsystem dependencies
        requires(Robot.driveTrain);
        this.filename = filename;
        this.first = first;
    ...
    public PathFinderReadInvertedAlpha(String filename) {
        // Use requires() here to declare subsystem dependencies
        requires(Robot.driveTrain);
        this.filename = filename;
    }
    ...
    void invertSettings(boolean invert) {
		for(int i = 0; i < Robot.driveTrain.driveMotors.length; i++)
			Robot.driveTrain.driveMotors[i].setInverted(!invert);
		
		Robot.driveTrain.driveMotors[0].setSensorPhase(invert);
		Robot.driveTrain.driveMotors[2].setSensorPhase(!invert);	
    }
}
